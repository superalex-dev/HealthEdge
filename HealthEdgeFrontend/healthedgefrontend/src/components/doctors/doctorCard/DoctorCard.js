...
const DoctorCard = ({ doctor }) => {
  ...
  const timeSlots = ['10:30', '11:30', '12:30', '13:30', '14:30', '15:30', '16:30', '17:30'];
  ...
  const customFormatDate = (dateString) => {
    ...
    const formattedHour = ((localDate.getHours() + 11) % 12 + 1);
...
const handleSelect = (day, time) => {
  ...
  <div style={styles.mainContent}>
    <img style={styles.profileImage} src={doctor.imageUrl} alt={`${doctor.firstName} ${doctor.lastName} ID IS: ${doctor.id}`} />
    <h2>{doctor.firstName} {doctor.lastName}</h2>
    <p style={styles.specializationRegion}>{specialization.name} - s</p>
    <p style={styles.earliestSlot}>{`Най-ранен свободен час: ${formattedDate}`}</p>
      ...
      <div style={styles.modalBackdrop}>
        <div style={styles.modalContent}>
          ...
          {days.map(day => (
            <div key={day}>
              ...
              {timeSlots.map(time => (
                <button
                  key={`${day}-${time}`}
                  style={{ ...styles.timeSlot, ...(isTimeSlotTaken(day, time) && styles.timeSlotDisabled) }}
          ...
          <div style={{ display: 'flex', justifyContent: 'center', marginTop: '1rem'}}>
            <button style={styles.modalButton} onClick={() => setShowDropdown(false)}>Затвори</button>
          </div>
        </div>
      </div>
    )}
    {selectedDate && (
      <>
        ...
        <button style={styles.finalizeButton} onClick={() => setShowForm(true)}>
          Запази
        </button>
      </>
    )}
    ...
    {showForm && (
        ...
        <label style={formStyle.label} htmlFor="reason">Причина:</label>
        <input
        ...
        <label style={formStyle.label} htmlFor="additionalNotes">Допълнителни бележки към доктора:</label>
        <textarea
        ...
        <label style={formStyle.label} htmlFor="paymentMethod">Метод на плащане:</label>
        <select
        ...
        <div style={formStyle.buttonContainer}>
          <button style={formStyle.button} onClick={handleSaveAndClose}>Запази</button>
          <button style={formStyle.cancelButton} onClick={() => setShowForm(false)}>Отказ</button>
        </div>
      </div>
      )}
  </div>
